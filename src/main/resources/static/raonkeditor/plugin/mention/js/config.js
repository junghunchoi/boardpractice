G_KPlugin["mention"].config = {
     ...
     **/
    appliedMentionClass: 'KE_Mention',

    /** useListSort
     * 활성화된 멘션 리스트를 자동 정렬해주는 설정 입니다.
     * 자동 정렬되지 않기를 원하신다면 false 값을 넣어주시면 됩니다.
     * */
    useListSort: true,

    /**
     * 활성화된 멘션 리스트에서 사용자가 입력한 값에 매칭되는 리스트만 보여주는 설정입니다.
     * false 값을 넣어주시면 필터링 되지 않고 지정한 리스트 모두 보여집니다.
     **/
    useListFilter: true,
     ...
     * onLoadedMentionPlugin: 멘션 기능 로드 완료 시 호출되는 함수 입니다.
                              설정하지 않으면 RAONKEDITOR_OnLoadedMentionPlugin 함수가 호출 됩니다.
     * onEnableMentionEvent: @뒤에 문자 입력 시 멘션이 활성화 되면 호출되는 함수 입니다.
                             설정하지 않으면 RAONKEDITOR_OnEnableMention 함수가 호출 됩니다.
     * onClickedMentionListEvent: mention 리스트 UI에서 리스트 선택 시 발생되는 함수 입니다.
                                  설정하지 않으면 RAONKEDITOR_OnClickedMentionList 함수가 호출 됩니다.
     * onBeforeCreateMentionList: mention 리스트에서 각 사용자별 멘션이 만들어 지기 전에 발생하는 함수 입니다.
                                  설정하지 않으면 RAONKEDITOR_onBeforeCreateMentionList 함수가 호출 됩니다.
     **/
    event: {
